...
# Steno AI - Backend Deployment to AWS App Runner (V2 - Fixed Configuration)
# This script deploys the backend with a NEW service name to avoid stuck service

set -e

REGION="us-east-1"
PROJECT_NAME="steno-prod"
SERVICE_NAME="${PROJECT_NAME}-backend-v2"  # Using v2 to avoid conflict

echo "🚀 Deploying Backend to AWS App Runner (V2 - Fixed Configuration)"
echo "Region: $REGION"
...
cat > /tmp/apprunner-config-v2.json <<EOF
{
...
SERVICE_OUTPUT=$(aws apprunner create-service \
  --cli-input-json file:///tmp/apprunner-config-v2.json \
  --region $REGION)
...
while [ $ATTEMPT -lt $MAX_ATTEMPTS ]; do
  ...
  if [ "$STATUS" = "RUNNING" ]; then
    ...
    aws logs tail "$LOG_GROUP" --region $REGION --since 10m --format short 2>&1 | tail -30 || echo "⚠️  Logs not available"
    
    exit 1
  fi
  
  if [ $((ATTEMPT % 3)) -eq 0 ] && [ $ATTEMPT -gt 5 ]; then
    # Get service ID for CloudWatch logs
    SERVICE_ID=$(echo "$SERVICE_ARN" | awk -F'/' '{print $NF}')
    LOG_GROUP="/aws/apprunner/$SERVICE_NAME/$SERVICE_ID/service"
    ...
    echo ""
    echo "📋 Latest logs:"
    aws logs tail "$LOG_GROUP" --region $REGION --since 2m --format short 2>&1 | tail -10 || echo "  (logs not available yet)"
    echo ""
  fi
...
echo "5. Clean up the stuck v1 service when AWS support assists"
echo ""

# Clean up
rm -f /tmp/apprunner-config-v2.json
